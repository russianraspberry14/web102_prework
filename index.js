...
contributionsCard.innerHTML = GAMES_JSON.reduce((acc, game)=>{return acc + game.backers},0).toLocaleString();
...
raisedCard.innerHTML = `$${GAMES_JSON.reduce((acc, game)=>{return acc + game.pledged},0).toLocaleString()}`
...
const displayStr = unfundedGames === 1 ? `A total of ${totalRaised.toLocaleString()} has been raised for all games. There is 1 game that remains unfunded. Help us in funding this amazing game` : `A total of ${totalRaised.toLocaleString()} has been raised for all games. There are ${unfundedGames} games that remain unfunded. Help us in funding these amazing games!`;
...
firstElement.textContent = firstGame.name;
secondElement.textContent = secondGame.name;
